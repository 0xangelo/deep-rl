...
def a2c_kfac(
    ...
    **saver_kwargs
    ...
    kfac = kfac or {}
    kfac = {"eps": 1e-3, "pi": True, "alpha": 0.95, "kl_clip": 1e-3, "eta": 1.0, **kfac}
    ...
    if warm_start is not None:
        if ":" in warm_start:
            ...
            index = None
        config, state = SnapshotSaver(warm_start, latest_only=False).get_state(
            int(index)
        )
    ...
    for samples in trange(beg, end, stp, desc="Training", unit="step"):
        all_obs = torch.empty(
            (steps, n_envs) + ob_space.shape, dtype=_NP_TO_PT[ob_space.dtype.type]
        )
        all_acts = torch.empty(
            (steps, n_envs) + ac_space.shape, dtype=_NP_TO_PT[ac_space.dtype.type]
        )
